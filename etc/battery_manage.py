...
def batteryrecord2db(dbname: str):
    while (bsdict := battery_status())['plugged'].upper() == 'PLUGGED_AC':
        insertbattinfoitem2db(
            dbname, bsdict['percentage'], bsdict['temperature'])
        time.sleep(30)
...
#     tablename = "wcdelaynew"
    ...
    timedf = pd.DataFrame(
        table, columns=["appendtime", "percentage", "temperature"])
    ...
    if (tdfsize := timedfgrp.shape[0]) != 0:
        ...
        jujinmins = int((pd.to_datetime(time.ctime()) -
                         timedfgrp.index[-1]).total_seconds() / 60)
    else:
        jujinmins = 0
...
if __name__ == "__main__":
    ...
    dbnameouter = touchfilepath2depth(
        getdirmain() / "data" / "db" / f"batteryinfo.db")
