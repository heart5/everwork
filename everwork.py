...
brandlist = ['威龙', '好运', '心诺',  '农尔康', '君思', '珍可惠', '红帅', '先生', '好棒美', '三友', '君妃', '火卤卤',
             '花氏', '洞庭渔王', '相思', '凤凰园', '洁龙', '怡口湘', '阿林', '好巴食', '巧大娘', '老成都', '一品鱼舫',
             '香约', '朝启', '顶牛', '湘俚味', '红叶', '无穷', '快活嘴', '花心子', '蛋大厨', '绿野香', '佳宝',
             '湘宝王', '乡巴佬', '鑫之恋', '泰越精厨', '可可哥', '倍儿爽', '小鹏', '醉吃香', '津尝乐', '张师傅',
             '馋大嘴', '卫龙', '凤将军', '馋大嘴', '儿时代', '爽口佳', '新丰园', '津津友味', '鲜多鲜','U品部落',
...
for br in brandlist:
    ...
    # print(notefbxsdf)
    qrystr = "select 日期,strftime('%Y%m',日期) as 年月,customer.往来单位编号 as 客户编码,sum(金额) as 退货金额," \
    ...
    fenxiyueduibi(token, note_store, qrystr, '退货金额', notefbxsdf, notelxxsdf, cnx, pinpai=br, cum=True)

    # notelxkhdf = ['']
    notelxkhdf = readinisection2df(cfp, br+'kehuguidleixing', br+'客户图表')
    notefbkhdf = readinisection2df(cfp, br+'kehuguidfenbu', br+'客户图表')
    # print(notefbkhdf)
    qrystr = "select 日期,strftime('%Y%m',日期) as 年月,customer.往来单位编号 as 客户编码,count(*) as 退货客户数," \
             "sum(金额) as 金额," \
             "substr(customer.往来单位编号,1,2) as 区域 ,"  "substr(customer.往来单位编号,12,1) as 类型, " \
             "product.品牌名称 as 品牌 from xiaoshoumingxi, customer, product " \
             "where (customer.往来单位 = xiaoshoumingxi.单位全名) " \
    ...
    if len(br) > 0:
        qrystr += ' and (品牌 = \'%s\')' %br
    qrystr += ' group by 日期,客户编码 order by 日期'
    fenxiyueduibi(token, note_store, qrystr,'退货客户数', notefbkhdf, notelxkhdf, cnx, pinpai=br)
